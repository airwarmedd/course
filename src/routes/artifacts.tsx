...
export const Route = createFileRoute("/artifacts")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "The nine artifacts you build across the course. Each one feeds the next.",
      },
    ],
  }),
        ...
        <div className="mx-auto max-w-4xl px-6 py-14">
          <ol className="divide-y divide-border border-y border-border">
            {artifacts.map((a, i) => (
              <li
                key={i}
                className="grid grid-cols-12 gap-6 py-6 hover:bg-secondary/40 px-3 transition-colors"
              >
                <div className="col-span-3 md:col-span-2 mono-label">
                  Artifact {String(i + 1).padStart(2, "0")}
                </div>
                <div className="col-span-9 md:col-span-10 font-serif text-xl">{a}</div>
              </li>
            ))}
          </ol>
          <p className="mono-label mt-10 text-center">
            Maintain these after graduation. 10 min daily · one interview monthly · quarterly
            positioning review.
          </p>
        </div>
      </section>
